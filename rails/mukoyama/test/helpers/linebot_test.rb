...
class LinebotTest < ActionController::TestCase
  test "register to linebot" do
    ...
    assert_nil Address.find_by(address: 'dummy-userid')
    assert_equal "コードを確認できません。センサを登録してください。", bot.reply('invalid message')
    assert_equal "コードを確認しました。「test2」の通知をお送りします。", bot.reply('2-01af')
    assert_equal 1, Address.where(address: 'dummy-userid').count
  end

  test "ask to linebot" do
    bot = Linebot.new('dummy-userid',Device.where(id: 2))
    assert_equal "センサの「登録」「一覧」「解除」は、そのように話しかけてください。", bot.reply('こんにちは')
    tl = Temp.where(device_id: 2).order("dt desc").limit(1).first
    assert_equal "19時42分 「test2」気温3.5、湿度90.5%です。", bot.reply('今test2どんな？')

    Temp.destroy_all
    assert_equal "温度データが見つかりませんでした。", bot.reply('今test2どんな？')

    assert_equal "2-0000 test2 \n", bot.reply('一覧')
  end
