...
class QuizController: UIViewController, AVAudioPlayerDelegate {
    ...
    @IBAction func buttonPushed(sender: AnyObject) {
        ...
        if let t =  b.titleLabel!.text {
            ...
            if t == answer{
                ...
                if(sound_correct.isPlaying) { sound_correct.stop() }
                sound_correct.play()
            }else{
                view_state = 2
                ans_data.append(false)
                ansImageView.image = UIImage(named: "false")
                if(sound_wrong.isPlaying) { sound_wrong.stop() }
                sound_wrong.play()
            }
        }
    }
    ...
    func loadQuestions(name: String){
        if let path = Bundle.main.path(forResource: name, ofType: "txt") {
            ...
            do {
                ...
                let lines = d.components(separatedBy: "\n")
                for line in lines {
    ...
    func loadQuestion(i: Int) {
        ...
        option1.setTitle(row[idx[0]], for: .normal)
        option2.setTitle(row[idx[1]], for: .normal)
        option3.setTitle(row[idx[2]], for: .normal)
        option4.setTitle(row[idx[3]], for: .normal)
    }
    ...
    func createSoundPlayer(name: String) -> AVAudioPlayer?{
        ...
        do{
            let audioPlayer = try AVAudioPlayer(contentsOf: sound_url as URL)
    ...
    override func viewDidLoad() {
        ...
        sound_correct = createSoundPlayer(name: "se_maoudamashii_chime13") // correct
        sound_wrong = createSoundPlayer(name: "se_maoudamashii_onepoint33") // wrong
    }
    
    override func viewDidAppear(_ animated: Bool) {
        super.viewDidAppear(animated)
        
        start_time = Date.timeIntervalSinceReferenceDate
        loadQuestion(i: ans_count)
    }
...
//            self.view.sendSubviewToBack(ansImageView)
    ...
    func btn_disable(flg: Bool){
        option1.isUserInteractionEnabled = flg
        option2.isUserInteractionEnabled = flg
        option3.isUserInteractionEnabled = flg
        option4.isUserInteractionEnabled = flg
    }

    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "end") {
            let end_time = Date.timeIntervalSinceReferenceDate - start_time
            let checkAnswerController:CheckAnswerController = segue.destination as! CheckAnswerController
